# Makefile for bike_parts_viewer_app

SHELL = cmd.exe

WORKTREE  		:= ../bike_parts_viewer_app_gh
BRANCH    		:= gh-pages
BUILD_DIR 		:= build\web
...
build-web:
	flutter build web --release --base-href /bike_parts_viewer_app/
...
ifeq ($(CURRENT_BRANCH),$(DEPLOY_BRANCH))
	...
	git reset --hard && \
	git clean -fdx && \
	xcopy ..\bike_parts_viewer_app\$(BUILD_DIR)\* . /E /I /Y >nul && \
	git add . && \
